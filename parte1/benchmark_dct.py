...
def dct1d_naive(f):
    ...
    for k in range(N):
        ...
        c[k] = alpha * np.sum(f * np.cos(np.pi * k * (2 * np.arange(N) + 1) / (2 * N)))
    return c

# DCT 2D: prima faccio la DCT 1D su ogni colonna e poi su ogni riga 
def dct2d_naive(block):
    N = block.shape[0]
    temp = np.zeros((N,N))
    # DCT sulle colonne
    for j in range(N):
        temp[:, j] = dct1d_naive(block[:, j])
    result = np.zeros((N, N))
    # DCT sulle righe
    for i in range(N):
        result[i, :] = dct1d_naive(temp[i, :])
    return result
...
def verifica_caso_test():
    ...
    attesa_1d    = np.array([4.01e+02,  6.60e+00,  1.09e+02, -1.12e+02,
    ...
    print("  " + "-" * (len(header) - 2))
    for k in range(8):
        print(f"  {k:>3}  {attesa_1d[k]:>12.2f}  {calcolata_1d[k]:>12.2f}  {abs(calcolata_1d[k]-attesa_1d[k]):>10.4f}")
    print(f"\n  Errore massimo: {errore_1d:.4f}  ", end="")
    print("✓ OK" if errore_1d < 1.0 else "✗ ATTENZIONE")

    # confronto tra DCT 2D calcolata con la nostra funzione e quella di scipy
    calcolata_2d = dct2d_naive(block_8x8)
    scipy_2d     = dctn(block_8x8, norm='ortho')
    errore_2d    = np.max(np.abs(calcolata_2d - scipy_2d))

    print(f"\n  DCT 2D — blocco 8×8\n")
    print(f"  Elemento [0,0]  atteso:    1.11e+03")
    print(f"  Elemento [0,0]  calcolato: {calcolata_2d[0,0]:.2e}")
    print(f"  Errore max vs scipy:       {errore_2d:.2e}  ", end="")
    print("✓ OK" if errore_2d < 1e-6 else "✗ ATTENZIONE")
...
def plot_benchmark(N_values, tempi_naive, tempi_libreria):
    ...
    ax.semilogy(N_arr, tempi_naive,    'o-', color='steelblue',  linewidth=2, label='DCT2 naïve  (atteso: O(N³))')
    ax.semilogy(N_arr, tempi_libreria, 's-', color='tomato',     linewidth=2, label='DCT2 scipy  (atteso: O(N²logN))')

    # curve teoriche di riferimento
    ref_N3   = tempi_naive[0]    * (N_arr / N_arr[0])**3
    ref_N2lN = tempi_libreria[0] * (N_arr**2 * np.log(N_arr)) / (N_arr[0]**2 * np.log(N_arr[0]))

    ax.semilogy(N_arr, ref_N3,   '--', color='steelblue', alpha=0.4, label='Riferimento O(N³)')
    ax.semilogy(N_arr, ref_N2lN, '--', color='tomato',    alpha=0.4, label='Riferimento O(N²logN)')
